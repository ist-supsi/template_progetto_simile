  ...
  <div class="content">
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-8">
          <div
            v-for="ii in Array.from(
              Array(Object.entries(cards).length),
              (n, i) => i
            ).filter(e => !(e % 2))"
            class="row"
            :key="ii[0]"
            ...
            <div v-if="cards[ii]" :class="[cards[ii + 1] ? 'col-6' : 'col-12']">
              <stats-card>
                <div slot="header" class="icon-warning">
                  ...
                  <i
                    :class="getCardIcon(cards[ii].name)"
                    data-toggle="tooltip"
                    title="cards[ii].title||'no data'"
                  ></i>
                </div>
                <div slot="content">
                  <p
                    v-if="!(cards[ii] && cards[ii].title)"
                    class="card-category placeholder-glow"
                  >
                  ...
                  <h4 class="card-title">
                    {{ cards[ii].data || "N.P." }} {{ cards[ii].uom || "" }}
                  </h4>
                  <p class="card-category">
                    {{
                      (cards[ii].message && guessLocLabel(cards[ii].message)) ||
                        "--"
                    }}
                  </p>
                  <p class="card-text">
                    <small class="text-muted">{{ cards[ii].type }}</small>
                  </p>
                </div>
                <div slot="footer">
                  <i
                    v-if="cards[ii].data === null"
                    class="fa fa-refresh fa-spin"
                  ></i>
                  <i
                    v-if="cards[ii].data === undefined"
                  ...
                  <i
                    ...
                    aria-hidden="true"
                  ></i
                  >{{ cards[ii].time && cards[ii].time.date }}
                  <i
                    v-if="cards[ii].time && cards[ii].time.time"
                    class="fa fa-clock-o"
                    aria-hidden="true"
                  ></i
                  >{{ cards[ii].time && cards[ii].time.time }}
                </div>
              </stats-card>
            </div>
            <div v-if="cards[ii + 1]" class="col-6">
              <stats-card>
                <div slot="header" class="icon-warning">
                  <!-- <i class="nc-icon nc-chart text-warning"></i> -->
                  <i
                    :class="getCardIcon(cards[ii + 1].name)"
                    data-toggle="tooltip"
                    title="cards[ii+1].title||'no data'"
                  ></i>
                </div>
                <div slot="content">
                  <p
                    v-if="!cards[ii + 1].title"
                    class="card-category placeholder-glow"
                  >
                    <span class="placeholder col-12"></span>
                  </p>
                  ...
                  <p class="card-category">
                    {{
                      (cards[ii + 1].message &&
                        guessLocLabel(cards[ii + 1].message)) ||
                        "--"
                    }}
                  </p>
                  <p class="card-text">
                    <small class="text-muted">{{ cards[ii + 1].type }}</small>
                  </p>
                </div>
                <div slot="footer">
                  <i
                    v-if="cards[ii + 1].data === null"
                    class="fa fa-refresh fa-spin"
                  ></i>
                  <i
                    v-if="cards[ii + 1].data === undefined"
                    class="fa fa-exclamation-triangle"
                  ></i>
                  <i
                    v-if="cards[ii + 1].time"
                    class="fa fa-calendar"
                    aria-hidden="true"
                  ></i
                  >{{ cards[ii + 1].time && cards[ii + 1].time.date }}
                  <i
                    v-if="cards[ii + 1].time && cards[ii + 1].time.time"
                    class="fa fa-clock-o"
                    aria-hidden="true"
                  ></i
                  >{{ cards[ii + 1].time && cards[ii + 1].time.time }}
                </div>
              </stats-card>
            </div>
          </div>
                ...
                <div class="input-group input-group-sm mb-3">
                  <div class="input-group-prepend">
                    ...
                    >
                      <i
                        ...
                        title="Scopri come interagire con la mappa"
                      ></i>
                    </button>
                  </div>
                  <select
                  ...
                  >
                    <option
                      ...
                      :key="feature.properties.foi_name"
                    >
      ...
      <ul class="nav nav-tabs" id="myTab" role="tablist">
        ...
        <li class="nav-item">
          <a
            :class="{
              'nav-link': true,
              active: selectedTab == 'cipais',
              enabled: selectedCipaisProcedures.length == 0
            }"
            id="cipais-tab"
            data-toggle="tab"
            role="tab"
            aria-controls="cipais"
            aria-selected="true"
            @click="selectedTab = 'cipais'"
            >Indicatori CIPAIS</a
          >
        </li>
        <li class="nav-item">
          <a
            :class="{
              'nav-link': true,
              active: selectedTab == 'satellitari',
              enabled: selectedSatelliteProcedures.length == 0
            }"
            id="satellitari-tab"
            data-toggle="tab"
            role="tab"
            aria-controls="satellitari"
            aria-selected="true"
            @click="selectedTab = 'satellitari'"
            >Dati satellitari</a
          >
        </li>
      </ul>
      ...
      <div class="tab-content" id="myTabContent">
        ...
        >
          ...
          >
            <div class="row">
              <h4>
                Non sono disponibili misure da sensore per la stazione
                selezionata.
              </h4>
            </div>
            <div v-if="features.features" class="form-group row">
              <label for="inputGroupSelect03" class="form-text text-muted"
                >Scegli un'altra località tra quelle monitorate da
                sensori:</label
              >
              <div class="col-sm-6">
                <select
                ...
                >
                  <option
                    v-for="feature in features.features.filter(feat =>
                      feat.properties.names.some(
                        nfo =>
                          !(
                            nfo.procedure.includes('CIPAIS') ||
                            nfo.procedure.startsWith('SATELLITE')
                          )
                      )
                    )"
                    :value="feature.properties.markerIndex"
          ...
          <div v-else>
            ...
            <p class="description text-justify">
              I dati provengono da sensori in-situ collocati su boe (laghi
              Maggiore e Como) e piattaforme (Lago di Lugano). I dati sono
              raccolti a frequenza elevata (sub-oraria) e trasmessi in tempo
              quasi reale. I sensori utilizzati sono di diversa tipologia a
              seconda della proprietà misurata. Nel caso dei pigmenti algali
              (clorofilla, ficocianina e ficoeritrina) si utilizzano sensori di
              tipo fluorimetrico. I sensori sono soggetti a periodiche
              operazioni di manutenzione (pulizia, calibrazione). I dati
              raccolti sono inoltre periodicamente validati mediante campagne di
              misura e analisi di laboratorio.
            </p>
            ...
            <div class="col-12">
              <data-table
                ...
                :translate="{
                  nextButton: '>',
                  previousButton: '<',
                  placeholderSearch: 'Cerca nella tabella'
                }"
        ...
        <div
          :class="{
            'tab-pane': true,
            fade: true,
            show: selectedTab == 'cipais',
            active: selectedTab == 'cipais'
          }"
          id="cipais"
          role="tabpanel"
          aria-labelledby="cipais-tab"
        >
          <div v-if="dataCipais.length == 0">
            <div class="row">
              <h4>
                Non sono presenti indicatori CIPAIS per la stazione selezionata
              </h4>
            </div>

            <div v-if="features.features" class="form-group row">
              <label for="inputGroupSelect03" class="form-text text-muted"
                >Scegli un'altra località:</label
              >
              <div class="col-sm-6">
                <select
                  name="prova"
                ...
                >
                  <option
                    v-for="feature in features.features.filter(feat =>
                      feat.properties.names.some(nfo =>
                        nfo.procedure.includes('CIPAIS')
                      )
                    )"
                    :value="feature.properties.markerIndex"
          ...
          <div v-else class="container-fluid">
            ...
            <p class="description text-justify">
              I dati degli “Indicatori CIPAIS” provengono dalle campagne
              limnologiche svolte sui laghi Maggiore e Lugano e finanziate dalla
              Commissione internazionale per la protezione delle acque
              italo-svizzere (CIPAIS). La Commissione si occupa di problemi
              quali l'inquinamento o altre alterazioni delle acque dei laghi
              Maggiore e di Lugano, nonché dei corsi d'acqua che segnano il
              confine o che lo attraversano. Le ricerche promosse dalla
              Commissione hanno lo scopo di proporre ai Governi contraenti i
              provvedimenti necessari per il risanamento delle acque comuni e la
              prevenzione dell'insorgenza di ulteriori forme di inquinamento.
              Contribuiscono inoltre ad integrare e approfondire le attività di
              monitoraggio e controllo effettuate dalle Istituzioni locali.
              <br />Per ulteriori informazioni:<a
                href="https://www.cipais.org/"
                target="_blank"
              >
                Sito Cipais</a
              >
            ...
            <div
              v-for="cc in loopOnPairs(
                Array.from(Array(dataCipais.length), (n, i) => i)
              )"
              class="row"
              :key="cc[0]"
            >
        ...
        <div
          :class="{
            'tab-pane': true,
            fade: true,
            show: selectedTab == 'satellitari',
            active: selectedTab == 'satellitari'
          }"
          id="satellitari"
          role="tabpanel"
          aria-labelledby="satellitari-tab"
        >
          <div v-if="dataSatellite.length > 0" class="container-fluid">
            <h4>Cosa sono i dati satellitari</h4>

            <p class="description text-justify">
              I grafici rappresentano le serie storiche di alcune statistiche
              derivate da immagini satellitari per ognuno dei parametri di
              qualità delle acque dei laghi (concentrazione di clorofilla-a,
              solidi sospesi totali e temperatura superficiale). Le statistiche
              che vengono calcolate sono la media, il primo e il terzo quartile,
              e la deviazione standard. I laghi possono essere divisi in più
              bacini, e per ogni bacino vengono calcolate le statistiche dei
              valori ottenuti a partire dalle mappe dei parametri, che vengono
              prodotte utilizzando le immagini acquisite dalle missioni ESA
              Sentinel-3 e NASA Landsat 8. Le mappe complete sono disponibili al
              seguente
              <a href="https://www.webgis.eo.simile.polimi.it/" target="_blank">
                link</a
              >
            </p>
            <div
              v-for="cc in loopOnPairs(
                Array.from(Array(dataSatellite.length), (n, i) => i)
              )"
              class="row"
              :key="cc[0]"
            >
                            ...
                            <div class="modal-content">
                              <div class="modal-body">
                                <highcharts
                                  ...
                                  :options="dataSatelliteWithQQ[cc[0]]"
                                ></highcharts>
                              </div>
                          ...
                          <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                              <div class="modal-body">
                                <highcharts
                                  :constructor-type="'stockChart'"
                                  :options="dataSatelliteWithQQ[cc[1]]"
                                ></highcharts>
                              </div>
                              <div class="modal-footer">
                                <button
          ...
          <div v-else>
            <div class="row">
              <h4>
                Non sono presenti dati satellitari per la stazione selezionata
              </h4>
            </div>
            <div v-if="features.features" class="form-group row">
              <label for="inputGroupSelect03" class="form-text text-muted">
...
export default {
    ...
    return {
      ...
      tableColumns2: [
        ...
        },
        {
          ...
          event: "click",
          handler: data => {
            const procedure = this.features.features[
              this.selectedMarker
            ].properties.names.filter(value =>
              data.procedures.includes(value.procedure)
            )[0];
      ...
      wmsUrl:
        "https://www.gishosting.gter.it/lizmap-web-client/lizmap/www/index.php/lizmap/service/?repository=dorota&project=cartografia_simile&",
  ...
  mounted() {
    ...
    this.istsos = this.vareseIstosos;
    ...
    const groupBy = (x, f, g) =>
      x.reduce((a, b) => {
        ...
        if (a[f(b)]) {
          ...
          if (
            !a[f(b)][rr.name] &&
            !["_1Q", "_3Q", "_SD"].some(suff => rr.procedure.endsWith(suff))
          ) {
            a[f(b)][rr.name] = rr;
          }
          return a;
            ...
            names: Object.entries(v)
              .sort((a, b) => {
                ...
                if (ia == ib) {
                  ...
                  if (b[1].procedure.toLowerCase().includes("cipais"))
                    return -1;
                  return 0;
                }
                if (ia > ib) {
  ...
  methods: {
    ...
    selectedTabObs() {
      if (this.tableData.data && this.tableData.data.length > 0) {
        ...
        this.selectedSatelliteProcedures &&
        this.selectedSatelliteProcedures.length > 0
      ) {
        this.selectedTab = "satellitari";
      } else if (
        this.selectedCipaisProcedures &&
        this.selectedCipaisProcedure.length > 0
      ) {
        this.selectedTab = "cipais";
      }
    },
    ...
    getLocLabel(feature) {
      ...
      const labels = ["Gandria", "Figino", "Ceresio Nord", "Ceresio Sud"];
    ...
    getCardIcon(name) {
      if (
        indicatorDescription.indicatorDescription[name] == undefined ||
        indicatorDescription.indicatorDescription[name].icon == undefined
      ) {
        ...
        if (info.samplingTime.beginposition && info.samplingTime.endposition) {
          ...
          const prm = this.istsos
            ...
            .then(response => {
              ...
              const variableAverage = mean(
                result.options.series[0].data.map(xy => xy[1])
              );
              result.options.yAxis.plotLines = [];

              if (indicatorDescription.indicatorDescription[proc.name]) {
                result.options.yAxis.reversed =
                  indicatorDescription.indicatorDescription[proc.name]
                    .reversed === true;
                if (
                  indicatorDescription.indicatorDescription[proc.name].limite
                    .lugano != null
                ) {
                ...
                if (
                  indicatorDescription.indicatorDescription[proc.name].obiettivo
                    .lugano != null
                ) {
                  result.options.yAxis.plotLines.push({
                    color: "green",
              ...
              const uom =
                !result.uom || result.uom == "null" ? "" : ` (${result.uom})`;

              if (
                info.observedproperties[0].name in
                indicatorDescription.indicatorDescription
              ) {
                result.options.title.text =
                  indicatorDescription.indicatorDescription[
                    info.observedproperties[0].name
                  ].title;
                result.options.subtitle.text = `${
                  indicatorDescription.indicatorDescription[
                    info.observedproperties[0].name
                  ].breveDescrizione
                }${uom}`;
    ...
    loadCardsData() {
      ...
      function updateCard(index, result) {
        ...
        if (cards[index] == undefined) {
          cards[index].title =
            indicatorDescription.indicatorDescription[cards[index].name].title;
        } else if (
          indicatorDescription.indicatorDescription[cards[index].name] ==
          undefined
        ) {
          cards[index].title = cards[index].description
            ? cards[index].description.substring(0, 27)
            : "";
        } else {
          cards[index].title =
            indicatorDescription.indicatorDescription[cards[index].name].title;
        ...
        if (result.procedure.includes("CIPAIS")) {
          ...
          cards[index].type = "Dato da Sensore";
        }

        cards[index].data = result.value;

        cards[index].uom =
          !result.uom || result.uom == "null" ? null : result.uom;

        if (result.x) {
          if (
            result.procedure.includes("CIPAIS") ||
            result.procedure.includes("ARPA")
          ) {
            cards[index].time = {
            ...
            cards[index].time = {
              date: result.x.toLocaleDateString("it-IT", {
                day: "2-digit",
                month: "2-digit",
                year: "2-digit"
              }),
              time: result.x.toLocaleTimeString("it-IT", {
                hour: "2-digit",
                minute: "2-digit"
              })
            };
      ...
      let calls = [];
      self.features.features[self.selectedMarker].properties.names.forEach(
        (info, cidx) => {
          ...
          info.observedproperties.forEach((nfo, idx) => {
            if (calls.length < 6) {
              ...
              calls.push(
                this.istsos.fetchBy(nfo.def, info.procedure).then(result => {
    ...
    areaLayerOptions() {
      return sharedFunctions.areaLayerOptions(this);
    },
    markerLayerOptions() {
      return sharedFunctions.markerLayerOptions(this);
    },
    removeMarker(index) {
    ...
    displayRow(data) {
      ...
      if (indicatorDescription.indicatorDescription[data.name]) {
        const message =
          indicatorDescription.indicatorDescription[data.name].description;
      } else {
    ...
    tableFetchData2() {
      ...
      return Promise.all([
        ...
        tableAllData.forEach(el => {
          const ff = results[1].data.data.filter(
            proc => proc.observedproperties[0].name == el.name
          );
          const ends = ff.map(proc => proc.samplingTime.endposition);
          const begins = ff.map(proc => proc.samplingTime.beginposition);
          const begin = begins.length
            ? Math.min(...begins.map(bb => Date.parse(bb)))
            : 0;
          const end = ends.length
            ? Math.min(...ends.map(bb => Date.parse(bb)))
            : 0;
    ...
    // },
    tableSetDataCipais() {
      const selectedProc = this.features.features[this.selectedMarker]
        .properties.names;
      this.selectedCipaisProcedures = selectedProc.filter(el =>
        el.procedure.includes("CIPAIS")
      );
    },
    tableSetDataSatellite() {
      const selectedProc = this.features.features[this.selectedMarker]
        .properties.names;
      this.selectedSatelliteProcedures = selectedProc.filter(el =>
        el.procedure.includes("SATELLITE")
      );
    },
    ...
    populateCockpit() {
      var self = this;
      this.istsos.fetchLastTemetature("MeteoMTX").then(result => {
      ...
      this.istsos.fetchLastO2s("AT500").then(result => {
        // self.last_temperature_data = result.options;
      ...
      for (const procedure of procedures) {
        this.istsos.fetchTemperatureSeries(procedure).then(result => {
          ...
          if (!self.series_data.series) {
            ...
            self.series_data.series.sort((el1, el2) => {
              el1.name < el2.name;
            });
          }
        });
